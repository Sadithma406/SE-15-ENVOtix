...
const OrganicBinScreen = ({ navigation, route }) => {
  // 1. Catch the userId passed from the Monitor Page
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      ...
      try {
        setLoading(true);
        // 2. Get the user's RFID dynamically from the database
        const userResponse = await fetch(`http://localhost:5000/api/users/${userId}`);
        const userData = await userResponse.json();

        if (userData && userData.RFID) {
          const userRFID = userData.RFID; // Matches "B21938cis9" dynamically

          // 3. Fetch organic bin levels using that specific RFID
          const binResponse = await fetch(`http://localhost:5000/api/bins/${userRFID}`);
          const binData = await binResponse.json();

          if (binData && binData.organic) {
            setFillLevel(binData.organic.fill_level);
            setLastUpdated(`Last updated: ${new Date(binData.organic.last_updated).toLocaleTimeString()}`);
          }
        }
      } catch (error) {
        console.error("Error fetching organic bin data:", error);
...
const styles = StyleSheet.create({
  ...
  recycleImage: { width: 35, height: 35, marginRight: 12 },
  iconTitleSection: { flexDirection: 'row', alignItems: 'center', marginBottom: 10, paddingVertical: 5 },
  iconSectionText: { fontSize: 18, fontWeight: 'bold', color: '#333' },
  scrollContent: { padding: 20 },
  mainCard: { backgroundColor: 'white', borderRadius: 25, paddingTop: 20, paddingBottom: 30, alignItems: 'center', elevation: 3 },
