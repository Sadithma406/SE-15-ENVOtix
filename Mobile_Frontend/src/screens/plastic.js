      ...
      try {
        ...
        if (userData && userData.RFID) {
          ...
          if (binData && binData.plastic) {
            setFillLevel(binData.plastic.fill_level);
            setLastUpdated(`Last updated: ${new Date(binData.plastic.last_updated).toLocaleTimeString()}`);
          }
        }
      } catch (error) {
...
const styles = StyleSheet.create({
  ...
  subHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: 25, marginTop: 30 },
